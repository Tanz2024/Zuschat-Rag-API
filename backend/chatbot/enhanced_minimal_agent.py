...
class EnhancedMinimalAgent:
    ...
    def format_outlet_response(self, outlets: List[Dict]) -> str:
        ...
        response = f"Perfect! I found {len(outlets)} ZUS Coffee outlet{'s' if len(outlets) > 1 else ''} for you: "
        
        outlet_details = []
        for i, outlet in enumerate(outlets, 1):
            detail = f"{i}. **{outlet['name']}** at {outlet['address']} (Hours: {outlet['hours']})"
            outlet_details.append(detail)
        
        response += " | ".join(outlet_details)
        response += " Would you like more information about services or contact details?"
        
        return response
    
    async def process_message(self, message: str, session_id: str) -> Dict[str, Any]:
        """Process message with enhanced keyword matching."""
        try:
            # Store session
            ...
            # Calculation detection (improved)
            elif any(op in message for op in ['+', '-', '*', '/', 'calculate', 'add', 'multiply']) or re.search(r'\d+.*\d+', message):
                try:
                    # Extract numbers and operations
                    if 'calculate' in message_lower:
                        # Remove "calculate" and try to process the rest
                    ...
                    if '+' in calc_part:
                        ...
                        if len(numbers) >= 2:
                            ...
                            response = "I need two numbers to add. Please provide something like '25.50 + 18.90' or 'add 15 to 20'."
                        ...
                        if len(numbers) >= 2:
                            ...
                            response = "I need two numbers to multiply. Please provide something like '23 * 4' or 'multiply 12 by 5'."
                        ...
                        if percent_match:
                            ...
                            if percent > 0 and value > 0:
                                ...
                                response = "Please provide a percentage calculation like '15% of 200' or '10 percent of 50'."
                        else:
                            response = "Please provide a percentage calculation like '15% of 200' or '10 percent of 50'."
                    else:
                        response = "I can help with calculations! Please provide expressions like '25 + 15', '23 * 4', or '15% of 200'."
                except:
                    response = "I had trouble with that calculation. Please use simple expressions like '25 + 15', '23 * 4', or '15% of 200'."
            
            # Product search (enhanced with real keywords)
            elif any(word in message_lower for word in ["product", "tumbler", "cup", "mug", "bottle", "drink"]):
                matching_products = self.find_matching_products(message)
                response = self.format_product_response(matching_products)
            
            # Outlet search (enhanced with real keywords)
            elif any(word in message_lower for word in ["outlet", "location", "store", "branch", "klcc", "pavilion", "sunway", "selangor", "kuala lumpur", "kl"]):
            ...
            # Default helpful response
                ...
                response = "I want to help you! I can assist with ZUS Coffee product information, outlet locations and hours, pricing calculations, or general inquiries. Could you please rephrase your question?"
